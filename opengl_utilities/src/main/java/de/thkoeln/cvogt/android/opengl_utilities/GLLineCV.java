...
public class GLLineCV {
    ...
    public GLLineCV(String id, float[] point1, float[] point2) {
        this(id,point1,point2,GraphicsUtilsCV.black);
    }
    ...
    /**
     * Flips/mirrors the line in the x, y, and/or z dimension
     ...
     * do not modify the vertex coordinates of the shape's triangles and lines
     * but define a model matrix which places the whole shape into the "real world".
     * @param flipX Specifies if the line shall be flipped in the x dimension.
     * @param flipY Specifies if the line shall be flipped in the y dimension.
     * @param flipZ Specifies if the line shall be flipped in the z dimension.
     */

    synchronized public void flip(boolean flipX, boolean flipY, boolean flipZ) {
        if (flipX) {
            this.point1[0] = -this.point1[0];
            this.point2[0] = -this.point2[0];
        }
        if (flipY) {
            this.point1[1] = -this.point1[1];
            this.point2[1] = -this.point2[1];
        }
        if (flipZ) {
            this.point1[2] = -this.point1[2];
            this.point2[2] = -this.point2[2];
        }
    }
