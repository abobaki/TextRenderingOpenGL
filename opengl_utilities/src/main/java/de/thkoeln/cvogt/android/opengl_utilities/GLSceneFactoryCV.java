...
public class GLSceneFactoryCV {
    ...
    public static GLShapeCV[] makeCubesInLine(int numberOfCubes, float scalingFactor, float[] point1, float[] point2, float[][] colors, boolean rotate, int animationDuration) {
        ...
        float[] y_axis = {0,1,0};
        float[] rotAxis = GraphicsUtilsCV.crossProduct3D(y_axis,vector);
        float rotAngle = (float) Math.toDegrees(Math.acos(GraphicsUtilsCV.dotProduct3D(y_axis, GraphicsUtilsCV.getNormalizedVectorCopy3D(vector))));
        float[][] positions = GraphicsUtilsCV.pointsInLine3D(point1,point2,numberOfCubes);
        for (int i=0; i<numberOfCubes; i++) {
            cubes[i] = GLShapeFactoryCV.makeCube("Cube"+i, colors);
            if (cubes[i]==null) return null;
            cubes[i].setScale(scalingFactor);
            if (animationDuration>0)
                cubes[i].addAnimator(GLAnimatorFactoryCV.makeAnimTransLinearTo(positions[i], animationDuration));
            else
    ...
    public static GLShapeCV[] makeSpheresOnCircle(int numberOfSpheres, float scalingFactor, float[] center, float radius, float[] perpendicularVector, float[][] colors, int animationDuration) {
        ...
        for (int i=0; i<numberOfSpheres; i++) {
            ...
            if (animationDuration>0)
                spheres[i].addAnimator(GLAnimatorFactoryCV.makeAnimTransLinearTo( positions[i], animationDuration));
            else
            ...
            for (int yPos=0; yPos<grid[0].length; yPos++)
                for (int zPos=0; zPos<grid[0][0].length; zPos++)
                    if (grid[xPos][yPos][zPos]) {
                        ...
                        if (animationDuration>0)
                            shape.addAnimator(GLAnimatorFactoryCV.makeAnimTransLinearTo(position, animationDuration));
                        else
            ...
            for (int yPos=0; yPos<grid[0].length; yPos++)
                for (int zPos=0; zPos<grid[0][0].length; zPos++)
                    if (grid[xPos][yPos][zPos]) {
                        ...
                        lines.addAll(Arrays.asList(GLShapeFactoryCV.linesForWireframeCuboid(((-grid.length+1)/2.0f+xPos)*edgeLength, ((-grid[xPos].length+1)/2.0f+yPos)*edgeLength, ((-grid[xPos][yPos].length+1)/2.0f+zPos)*edgeLength,edgeLength,edgeLength,edgeLength,GraphicsUtilsCV.red)));
                    }
